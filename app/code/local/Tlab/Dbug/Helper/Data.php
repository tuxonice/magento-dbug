...
class Tlab_Dbug_Helper_Data extends Mage_Core_Helper_Abstract{

    protected $xmlDepth=array();
    protected $xmlCData;
    protected $xmlSData;
    protected $xmlDData;
    ...
    protected $xmlAttrib;
    protected $xmlName;
    protected $arrType=array("array","object","resource","boolean","NULL");
    protected $bInitialized = false;
    protected $bCollapsed = false;
    ...
    public function deBug($var,$forceType="",$bCollapsed=false) {
        //include js and css scripts
            if(!defined('BDBUGINIT')) {
                ...
                $this->cssJsData = $this->initJSandCSS();
            }
    ...
    public function output(){
        ...
        return $this->cssJsData.$this->outputData;
    ...
    public function backtraceLog(){

        $trace = debug_backtrace();
        //var_dump($trace); exit; return;
        if(is_array($trace))
            foreach($trace as $key=>$value) {
                $trace[$key]['file'] = str_replace($_SERVER['DOCUMENT_ROOT'], '', $trace[$key]['file']);
                $trace[$key]['class'] = $this->getClassTree($trace[$key]['class']);
                if(isset($trace[$key]['object']))
                    $trace[$key]['object'] = $this->objectToArray($trace[$key]['object']);
            }

        $trace = json_encode($trace);
        $this->dbLog($trace, null, true);


    }


    public function backtraceToArray(){
        ...
        if(is_array($trace))
            foreach($trace as $key=>$value) {
                $backtrace[$key]['file'] = str_replace($basePath, '', $value['file']);
                $backtrace[$key]['function'] = $value['function'];
                $backtrace[$key]['line'] = $value['line'];
                $backtrace[$key]['class'] = $value['class'];
                if(isset($value['object']) && is_object($value['object']))
                    $backtrace[$key]['object'] = $this->objectToArray($value['object']);
                $backtrace[$key]['args'] = $value['args'];
            }
    ...
    public function objectToArray($className){
        ...
        $funcs = $reflectionClass->getMethods();
        foreach($funcs as $function)
            if($function->class == $className)
                $methods[] = $function->name;


        return array('name'=>$className,'properties'=>$properties, 'methods'=>$methods);
     ...
     * @param bool $forceLog
     */
    public function fileLog($message, $level = null, $file = 'dbug.log', $forceLog = false, $outputFormat = 'text')
    {
        if (!Mage::getConfig()) {
            return;
        }
        ...
        try {
            if (!isset($loggers[$file])) {
                ...
                {
                    $format = '{"time":"%timestamp%","priority":"%priorityName%","prioritynmb":"(%priority%)","content":%message%}' . PHP_EOL;
                }else
                {
     ...
     * @return array List of parent classes
     */
    ...
    public function dbLog($message, $level = null, $forceLog = false, $outputFormat = 'text'){
       ...
       $level  = is_null($level) ? Zend_Log::DEBUG : $level;
        ...
        try {
            ...
            $sql        = "INSERT INTO tlab_dbug_log (log_date, log_time, log_type, log_content) VALUES (:log_date, :log_time, :log_type, :log_content)";
            $stmt = $connection->prepare($sql);
            $stmt->bindValue(':log_time',date("H:i:s"), PDO::PARAM_STR);
            $stmt->bindValue(':log_date',date("Y:m:d"), PDO::PARAM_STR);
            $stmt->bindValue(':log_type',$level, PDO::PARAM_STR);
            $stmt->bindValue(':log_content',$message, PDO::PARAM_STR);
            $stmt->execute();
    ...
    public function readFromEndByLine($filename)
    {
